...
def parse_json(json_file, data_type):
    ...
    day_list = open(dst_dir + '/day/' + data_type + '.txt', 'w')
    night_list = open(dst_dir + '/night/' + data_type + '.txt', 'w')
    with open(json_file, 'r') as f:
        ...
        for img_data in tqdm.tqdm(read_data):
            ...
            if time_data == 'night':
                save_path = dst_dir + '/night/' + data_type + '/image'
                night_list.write(dst_dir + '/night/' + data_type + '/image/' + img_name + '\n')
            else:
                save_path = dst_dir + '/day/' + data_type + '/image'
                day_list.write(dst_dir + '/day/' + data_type + '/image/' + img_name + '\n')
            write_data = open(save_path + '/' + img_name.replace('.jpg', '') + '.txt', 'w')
            shutil.copy(data_dir + 'images/100k/' + data_type + '/' + img_name, save_path)
